...
3. Expo's official Metro tunnel (`expo start --tunnel`) on `*.exp.direct`

Scan the QR with [Expo Go](https://expo.dev/go) **SDK 57**. Expo Go verifies the packager hostname and only accepts Expo's tunnel hosts — a `trycloudflare.com` Metro URL shows "Hostname could not be verified".
